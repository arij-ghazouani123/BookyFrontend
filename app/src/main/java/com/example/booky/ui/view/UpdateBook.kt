...
class UpdateBook : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        txtEditSub.setText(subject.toString())
        txtEdit.setText(description.toString())
        amainIntent = Intent(this, MainActivity::class.java)
        btnADDQuestion.setOnClickListener{
            if (txtEditSub?.text!!.isEmpty()) {
                Toast.makeText(this@UpdateBook, "Title must not be empty", Toast.LENGTH_SHORT).show()

            return@setOnClickListener
        }
        if (txtEdit?.text!!.isEmpty()) {
            Toast.makeText(this@UpdateBook, "Description must not be empty", Toast.LENGTH_SHORT).show()

            return@setOnClickListener
        }
            val  mainIntent = Intent(this, UploadImageUpdate::class.java)
